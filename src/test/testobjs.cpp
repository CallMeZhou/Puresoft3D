	...
	: SceneObject(pipeline, parent)
{
	...
	m_programme = findOrCreateProgramme(PROCNAME(VertexProcesserTEST), PROCNAME(InterpolationProcessorTEST), PROCNAME(FragmentProcessorTEST));
...
void Earth::update(float timeSpanSec, const mat4& parent)
{
	...
	m_texTrans.translation(m_texTransX, 0, 0);
